...
local function get_visual_selection()
  ...
  if #lines == 0 then
    notify("unable to read visual selection", vim.log.levels.WARN)
    return nil
  end
...
local function apply_result(selection, text)
  ...
  api.nvim_buf_set_text(selection.bufnr, selection.start_row, selection.start_col, selection.end_row, selection.end_col, lines)
end
